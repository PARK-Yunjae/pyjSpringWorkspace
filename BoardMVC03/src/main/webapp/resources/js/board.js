...
console.log("글쓰기 테스트1");
    ...
    fetch("api/boards", {
        ...
        body: JSON.stringify({memID, title, content, writer })
    })
    .then(loadList)
    .catch(error => {
        console.error("Error occurred while processing the request:", error);
...
function goContent(idx) {
    ...
    const count = document.querySelector(`#cnt${idx}`);
    if (contentRow.style.display === "none") {
        fetch(`api/boards/${idx}`)
            .then(response => response.json())
            .then(data => {
            	count.innerText = parseInt(count.innerText) + 1;
                document.querySelector(`#ta${idx}`).value = data.content; // textarea
            })
            .catch(error => {
                console.error("Error occurred while processing the request:", error);
            });
